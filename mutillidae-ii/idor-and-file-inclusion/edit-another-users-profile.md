# Edit Another User's Profile

## Lab 18: Insecure Direct Object References - Edit Another User's Profile

<figure><img src="../../.gitbook/assets/image (27) (1) (1) (1) (1) (1).png" alt=""><figcaption></figcaption></figure>

Go to user profile page: [https://127.0.0.1/index.php?page=view-account-profile.php\&uid=1](https://127.0.0.1/index.php?page=view-account-profile.php\&uid=1)

Only changing the uid value parameter to 5 we can see the relative user: [http://127.0.0.1/index.php?page=view-account-profile.php\&uid=5](http://127.0.0.1/index.php?page=view-account-profile.php\&uid=5)

<figure><img src="../../.gitbook/assets/image (32) (1) (1) (1) (1).png" alt=""><figcaption></figcaption></figure>
